...
import (
	...
	"github.com/cyril-jump/gophkeeper/internal/server/app/handlers/http/create-new-blob-data/adapters"
	"github.com/cyril-jump/gophkeeper/internal/server/app/handlers/http/create-new-blob-data/usecase"
...
func (suite *Suite) Test_CreateNewBlobData_200() {
	suite.e.POST("/", suite.reqs.CreateNewBlobData)
	suite.db.EXPECT().CreateNewBlobData(gomock.Any(), gomock.Any(), gomock.Any()).Return(nil)
...
func (suite *Suite) Test_CreateNewBlobData_401() {
...
func (suite *Suite) Test_CreateNewBlobData_500() {
	suite.e.POST("/", suite.reqs.CreateNewBlobData)
	suite.db.EXPECT().CreateNewBlobData(gomock.Any(), gomock.Any(), gomock.Any()).Return(domain.ErrInternal)
	data := domain.BlobData{
		ID:       0,
		Data:     []byte("111"),
		Metadata: "222",
